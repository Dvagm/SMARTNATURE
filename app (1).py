...
# --- PERUBAHAN UTAMA: FUNGSI fetch_forecast() MENGAMBIL PARAMETER LOKASI ---
...
# --- PERUBAHAN UTAMA: FUNGSI get_summary_text() MENGAMBIL PARAMETER LOKASI ---
...
# =========================
def polling_loop(token, chat_override=None, lat=None, lon=None, location_name=None):
    """
        ...
        try:
            ...
            for update in updates:
                ...
                if text and text.strip().lower() == 'laporan':
                    ...
                    df_processed_polled = add_engineered_columns(fetch_forecast(lat, lon))
...
# --- PERUBAHAN UTAMA: Tambahkan widget untuk memilih lokasi ---
...
# --- Memuat ulang data berdasarkan lokasi yang dipilih ---
if 'data_frame' not in st.session_state or st.session_state.get('current_location') != location_name:
...
# --- PERUBAHAN UTAMA: Judul dasbor dinamis ---
...
if st.sidebar.button("▶️ Aktifkan Polling Bot Telegram"):
    if 'polling_thread' not in st.session_state or not st.session_state.get('polling_thread_alive', False):
        t = threading.Thread(target=polling_loop, args=(telegram_token, telegram_chat, LAT, LON, location_name), daemon=True)
        t.start()
...
with tab_ai:
    ...
    st.success(get_summary_text(df_processed, location_name).split("**Ringkasan Cuaca")[0] + "\n\n" + get_summary_text(df_processed, location_name).split("**Prediksi AI dan Rekomendasi Cuaca:")[1])
...
with tab_soil_data:
    ...
    if st.session_state.get('soil_data_loaded'):
        ...
        st.subheader("Rekomendasi Berdasarkan Kondisi Tanah")
